...
def count_corpus(tokens):  #@save
    ...
    if len(tokens) == 0 or isinstance(tokens[0], list):
        # 将词元列表展平成一个列表
        tokens = [token for line in tokens for token in line]
    return collections.Counter(tokens)  # Counter是一个字典子类，用于计数可哈希对象。它的键是词元，值是词元出现的次数。
...
def seq_data_iter_random(corpus, batch_size, num_steps):  #@save
    """使用随机抽样生成一个小批量子序列"""
    # 从随机偏移量开始对序列进行分区，随机范围包括num_steps-1，如果大于等于num_steps，则相当于抛弃了一个子序列
...
class RNNScratch(d2l.Module):  # 为RNNModelScratch的相同实现
...
class EncoderDecoder(d2l.Classifier):
    ...
    def forward(self, enc_X, dec_X, *args):
        ...
        # Return decoder output and state
...
def init_seq2seq(module):
    ...
    if type(module) == nn.Linear:
         nn.init.xavier_uniform_(module.weight)
    if type(module) == nn.GRU or nn.LSTM:
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        #     use_bias)
        # d2l包中的MultiHeadAttention类用num_hiddens参数包含了
        # key_size, query_size, value_size三个参数
        self.attention = d2l.MultiHeadAttention(
